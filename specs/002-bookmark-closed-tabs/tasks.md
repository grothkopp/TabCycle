...
- [x] T008 [US1] Modify the `runEvaluationCycle` function in src/background/service-worker.js to: (1) read `bookmarkEnabled` from settings (with default `true`), (2) if enabled, call `resolveBookmarkFolder(settings)` once before processing gone tabs, (3) for each tab that reached Gone status, before calling `chrome.tabs.remove()`, check `isBookmarkableUrl(tab.url)` and if true call `bookmarkTab(tab, folderId)` wrapped in try/catch, (4) proceed with `chrome.tabs.remove()` regardless of bookmark success. Import `resolveBookmarkFolder`, `isBookmarkableUrl`, `bookmarkTab` from `./bookmark-manager.js`. Tab info (url, title) must be captured via `chrome.tabs.get(tabId)` before removal since tabMeta does not store URLs.
...
- [x] T011 [US2] Modify the group-closing section of `runEvaluationCycle` in src/background/service-worker.js to: before calling `closeGoneGroups()`, if bookmarkEnabled is true, for each gone group: (1) query the group's title via `chrome.tabGroups.get(groupId)`, (2) query all tabs in the group via `chrome.tabs.query({ groupId })`, (3) call `bookmarkGroupTabs(groupTitle, tabs, folderId)` wrapped in try/catch, (4) proceed with group closure regardless of bookmark success. The existing individual-tab Gone handling (tabs in special "Red" group) already creates individual bookmarks from T008.
