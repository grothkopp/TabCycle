...
- Q: What happens when Chrome restores tabs from a previous session? → A: Restored tabs continue with the active time they had before closing. Tab metadata (including refresh times) persists across browser restarts and session restores. If Chrome delays session restore (for example, until after profile selection), startup reconciliation must wait for the live session state instead of overwriting preserved metadata with fresh state.
...
- Q: Should Chrome restore a suspended/discarded tab trigger a navigation event? → A: No. When Chrome restores a suspended or discarded tab, the resulting navigation should NOT reset the tab's age. The extension marks `tabs.onUpdated` events where `changeInfo.discarded === false` and suppresses the immediate follow-up navigation event, with `tab.discarded` / `tab.status === 'unloaded'` checks as fallback.
...
- What happens when Chrome restores a suspended or discarded tab? The extension marks `tabs.onUpdated` with `changeInfo.discarded === false` and skips the immediate restore-triggered navigation event, preserving the tab's existing age; `tab.discarded` / `tab.status === 'unloaded'` are also checked as fallback.
...
- What happens if `tabMeta` or `windowState` loses tabs or group references because Chrome delivered partial live state or reactive events raced? The extension continuously reconciles stored tracked state against live normal windows/tabs/groups during evaluation, debounced sorts, navigation resets, focus refreshes, and startup reconciliation. Missing live tabs are re-adopted, stale group/window references are pruned, and recovered tabs inherit the freshest evidence available from their group peers or stored group zone.
...
- **FR-025**: The extension MUST persist all tab metadata (refresh times, statuses) and the active-time accumulator across browser restarts and session restores. Restored tabs MUST continue with their previous active time and status, not reset to Green. Startup reconciliation MUST be non-destructive while Chrome's restored windows/tabs/groups are not yet available, and it MUST remap extension-managed Yellow/Red group tracking to the live group IDs once restore completes.
...
- **FR-046**: The navigation handler MUST skip restore-triggered events for suspended or discarded tabs to avoid resetting their age. It MUST mark `tabs.onUpdated` events where `changeInfo.discarded === false` and suppress the immediate follow-up navigation event, and also check `tab.discarded` or `tab.status === 'unloaded'` as fallback.
...
- **FR-048**: All tracked-state mutations that change `v1_tabMeta` or `v1_windowState` MUST be serialized through a single mutation queue so reactive handlers only apply add/remove/modify changes to the latest stored snapshot instead of overwriting tracked state from stale reads.
- **FR-049**: The extension MUST continuously self-heal tracked state by reconciling stored `tabMeta` and `windowState` against live normal windows, tabs, and tab groups during evaluation, debounced sorts, navigation resets, focus refreshes, and startup reconciliation. When re-adopting a missing live tab that already belongs to a tracked group, the extension MUST infer the recovered tab's minimum age/status from the freshest tracked peer in that group when available, otherwise from the stored group zone or managed-group type.
