...
const { AGE_CALCULATION_MODE, TAB_LIFECYCLE_STAGE } = await import('../../src/shared/constants.js');
const { findAllTabsNeedingStatusTransition } = await import('../../src/background/status-evaluator.js');
const { autoNameUnnamedGroupsWhenReady } = await import('../../src/background/group-manager.js');
...
describe('alarm-cycle integration', () => {
  ...
  it('should transition green tabs to yellow after greenToYellow threshold', () => {
    ...
    const tabMeta = {
      1: {
        ...
        status: TAB_LIFECYCLE_STAGE.GREEN, groupId: null, isSpecialGroup: false, pinned: false,
      },
    };

    const transitions = findAllTabsNeedingStatusTransition(tabMeta, 1500, settings);
  ...
  it('should transition yellow tabs to red after yellowToRed threshold', () => {
    ...
    const tabMeta = {
      1: {
        tabId: 1, windowId: 1, refreshActiveTime: 0, refreshWallTime: 0,
        status: TAB_LIFECYCLE_STAGE.YELLOW, groupId: null, isSpecialGroup: false, pinned: false,
      },
    };

    const transitions = findAllTabsNeedingStatusTransition(tabMeta, 2500, settings);
  ...
  it('should transition red tabs to gone after redToGone threshold', () => {
    ...
    const tabMeta = {
      1: {
        tabId: 1, windowId: 1, refreshActiveTime: 0, refreshWallTime: 0,
        status: TAB_LIFECYCLE_STAGE.RED, groupId: null, isSpecialGroup: false, pinned: false,
      },
    };

    const transitions = findAllTabsNeedingStatusTransition(tabMeta, 3500, settings);
  ...
  it('should not transition pinned tabs regardless of age', () => {
    ...
    const tabMeta = {
      1: {
        tabId: 1, windowId: 1, refreshActiveTime: 0, refreshWallTime: 0,
        status: TAB_LIFECYCLE_STAGE.GREEN, groupId: null, isSpecialGroup: false, pinned: true,
      },
    };
  ...
  it('should handle multiple tabs with mixed statuses in one evaluation', () => {
    ...
    const tabMeta = {
      1: { tabId: 1, windowId: 1, refreshActiveTime: 500, refreshWallTime: 0, status: TAB_LIFECYCLE_STAGE.GREEN, groupId: null, isSpecialGroup: false, pinned: false },
      2: { tabId: 2, windowId: 1, refreshActiveTime: 0, refreshWallTime: 0, status: TAB_LIFECYCLE_STAGE.GREEN, groupId: null, isSpecialGroup: false, pinned: false },
      3: { tabId: 3, windowId: 1, refreshActiveTime: 0, refreshWallTime: 0, status: TAB_LIFECYCLE_STAGE.YELLOW, groupId: null, isSpecialGroup: false, pinned: false },
    };
  ...
  it('should use wall clock mode when configured', () => {
    ...
    const transitions = findAllTabsNeedingStatusTransition(tabMeta, 0, settings);
    expect(transitions[1]).toEqual({ oldStatus: 'green', newStatus: 'yellow' });
  });
  ...
  it('should skip auto-naming before the configured delay is reached', async () => {
    ...
    const result = await autoNameUnnamedGroupsWhenReady(1, {}, windowState, {
      enabled: true,
      delayMinutes: 5,
      nowMs: now,
    });
